...
numFrames=6*ceil(frameRate/tFreq);
Bcol=128;
...
try
    ...
    final_contrast = 0.025;
    initial_contrast = 0.025; % 1
    for j = 1:nori
        for i = 1:numFrames
            phase=(i*tFreq/frameRate)*2*pi;
            contrast = final_contrast + (initial_contrast-final_contrast)*2^(-(i-1)/5);
