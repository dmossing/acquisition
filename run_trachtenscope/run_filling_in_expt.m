function run_filling_in_expt(varargin)
...
p.addParameter('bgsizes',20);
p.addParameter('sFreqs',0.08); % cyc/vis deg
p.addParameter('tFreqs',1); % cyc/sec
p.addParameter('position',[-10,0; 0,0; 10,0]);
p.addParameter('contrast',[0 0.015 0.03 0.06 0.12 0.25 0.5 1]);
p.addParameter('groundContrast',[0 0.25]);
...
try
    ...
    result.positioninds = 1:size(result.position,1);
    
    % create all stimulus conditions from the single parameter vectors
    nConds  =  [length(result.orientations) length(result.sizes) length(result.tFreqs) length(result.sFreqs) length(result.contrast) length(result.groundContrast) length(result.bgsizes) length(result.positioninds)];
    allConds  =  prod(nConds);
    % result.allConds = allConds;
    % repPerCond  =  allConds./nConds;
    conds  =  makeAllCombos(result.orientations,result.sizes,result.tFreqs,result.sFreqs,result.contrast,result.groundContrast,result.bgsizes,result.positioninds);
    ...
    PatchRadiusPix = [PatchRadiusPix; ceil(result.bgsizes.*wininfo.PixperDeg/2)]; % radius!!
    
    xpostemp = wininfo.xposStim(:);
    ypostemp = wininfo.xposStim(:);
    sizetemp = result.sizes(:)';
    xpostemp = repmat(xpostemp,1,size(sizetemp,2));
    ypostemp = repmat(ypostemp,1,size(sizetemp,2));
    sizetemp = repmat(sizetemp,size(xpostemp,1));
    x0 = floor(wininfo.xRes/2 + (xpostemp - sizetemp/2)*wininfo.PixperDeg);
    y0 = floor(wininfo.yRes/2 + (-ypostemp - sizetemp/2)*wininfo.PixperDeg);
    ...
    [gratingInfo.Orientation,gratingInfo.Contrast,gratingInfo.spFreq,...
        gratingInfo.tFreq, gratingInfo.Size, gratingInfo.groundContrast, gratingInfo.bgSize, gratingInfo.positionInd] = deal(zeros(1,allConds*result.repetitions));
    ...
    gratingInfo.widthLUT = [result.sizes(:); result.bgsizes(:)];
    gratingInfo.widthLUT = [gratingInfo.widthLUT(:) width(:)];
    ...
    if kinp == 'q'|kinp == 'Q',
        ...
        for itrial = 1:result.repetitions,
            ...
            while ~isempty(tmpcond)
                ...
                thisstim.tex = gen_parallel_gratings(wininfo,result.gratingInfo,thisstim);
...
% STOP ACQUISITION ON SCANBOX !!!
    ...
    function result = pickNext(result,trnum,thiscond)
        ...
        result.gratingInfo.bgSize(trnum) = thiscond(7);
        result.gratingInfo.positionInd(trnum) = thiscond(8);
    end
    ...
    function thisstim = getStim(gratingInfo,trnum)
        ...
        bin = (gratingInfo.widthLUT(:,1) == gratingInfo.bgSize(trnum));
        thisstim.thisbgwidth = gratingInfo.widthLUT(bin,2);
        thisstim.thisdeg = gratingInfo.Orientation(trnum);
        thisstim.thissize = gratingInfo.Size(trnum);
        thisstim.bgsize = gratingInfo.bgSize(trnum);
        ...
        thisstim.positionind = gratingInfo.positionInd(trnum);
