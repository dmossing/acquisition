...
scanRate = 1000;
delay = 1; %in seconds
numScans = 2000;  %2x the expected # of scans (2*scanRate*delayms/1000)
numScansRequested = 0;
...
try
    ...
    %Configure the analog input range on channel 0 for bipolar +-5 volts (LJ_rgBIP5V).
    LJ_rgBIP5V = ljudObj.StringToConstant('LJ_rgBIP5V');
    ljudObj.AddRequest(ljhandle, LabJack.LabJackUD.IO.PUT_AIN_RANGE, 0, LJ_rgBIP5V, 0, 0);
