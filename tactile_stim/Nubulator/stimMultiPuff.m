function [TrialInfo, SaveFile] = stimMultiPuff(SaveFile, varargin)
...
gd.Experiment.timing.stimOffset = 0.5;    % in seconds
gd.Experiment.timing.ITI = 2.5;             % in seconds
...
if hObject.Value
    try
        ...
        % Trigger pistons
        startTrig1 = max(floor(Experiment.params.samplingFrequency * Experiment.timing.ITI),1);    % start after ITI
        endTrig1 = Experiment.timing.numScansPerTrial-1; % -Experiment.params.samplingFrequency * Experiment.timing.stimOffset % end on last trigger of trial
        startTrig2 = max(floor(Experiment.params.samplingFrequency * (Experiment.timing.ITI+Experiment.timing.stimOffset)),1);    % start after ITI
        endTrig2 = Experiment.timing.numScansPerTrial-1;
        Experiment.PistonTrigger1 = zeros(Experiment.timing.numScansPerTrial, 1);
        Experiment.PistonTrigger1(startTrig1:endTrig1) = 1;
        Experiment.PistonTrigger2 = zeros(Experiment.timing.numScansPerTrial, 1);
        Experiment.PistonTrigger2(startTrig2:endTrig2) = 1;
        ...
        PistonTrigger = [Experiment.PistonTrigger1 Experiment.PistonTrigger2];
%         PistonTrigger2 = Experiment.PistonTrigger2;
    ...
    function QueueData(src,eventdata)
        ...
        if hObject.Value && any(numTrialsRemain) % user hasn't quit, and experiment hasn't finished
            ...
            if TrialInfo.StimID(currentTrial) ~= 0 % current trial is not control trial
                pc = PistonCombinations(TrialInfo.StimID(currentTrial),:);
