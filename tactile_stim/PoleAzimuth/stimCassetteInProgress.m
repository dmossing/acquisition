function [TrialInfo, SaveFile] = stimCassetteInProgress(SaveFile, varargin)
...
gd.Stimuli.list = uitable(...
    ...
    'Units',                'Normalized',...
    'Position',             [0,0,.6,.7],...
    'ColumnName',           {'angle','# per Block','Delete'},...
...
% NOT NEEDED, IF ANGLE SAMPLING IS ALWAYS ENABLED
...
% Gather inputs
% ANGLE CHOSEN ALWAYS TO BE ENABLED
% toggle = [gd.Stimuli.positionTable.Data{:,1}];
% if ~any(toggle)
...
stimParams = gd.Stimuli.positionTable.Data;
...
% end
stimuli = ((0:stimParams-1)*60 + 30)'; % assume stims are laid out evenly starting 30 deg CW
...
stimuli = gd.Stimuli.list.Data(:,1);
...
%     numPerBlock = gd.Stimuli.list.Data(:,3); % gather # of trials per block
    numPerBlock = gd.Stimuli.list.Data(:,2); % gather # of trials per block
    first = 1;
else
%     numPerBlock = [str2double(gd.Parameters.controlNum.String);gd.Stimuli.list.Data(:,3)]; % gather # of trials per block
    numPerBlock = [str2double(gd.Parameters.controlNum.String);gd.Stimuli.list.Data(:,2)]; % gather # of trials per block
    first = 0;
end
...
%         Experiment.stim.position = gd.Stimuli.list.Data(:,1:2);
        Experiment.stim.position = gd.Stimuli.list.Data(:,1);
        Experiment.stim.Distances = [0; 25; 15];
        Experiment.stim.numPositions = numel(Experiment.stim.position);
%         Experiment.stim.angleMove = str2double(gd.Controls.stepAngle.String);
%         Experiment.stim.numPerBlock = gd.Stimuli.list.Data(:,3);
        Experiment.stim.numPerBlock = gd.Stimuli.list.Data(:,2);
...
%         end
        ...
        if Experiment.params.catchTrials
            Experiment.stim.position = [nan(1,1); Experiment.stim.position];
        ...
        Experiment.Triggers = zeros(Experiment.timing.numScansPerTrial, numel(OutChannels), Experiment.params.catchTrials+Experiment.stim.numPositions);
        maxNumStepTriggers = size(moveStepperMotor(330, Experiment.params.samplingFrequency, 1, 2, false, 2),1);
        % Build stepper motor triggers
        for p=Experiment.params.catchTrials+(1:Experiment.stim.numPositions)
            stepTriggers = moveStepperMotor(Experiment.stim.position(p), Experiment.params.samplingFrequency, 1, 2, false, 2);
            numStepTriggers = size(stepTriggers, 1);
            numLinTriggers = Experiment.params.samplingFrequency; % allow 1 sec for linear motor to move 
...
%             endTrig = Experiment.timing.numScansPerTrial-numStepTriggers+1;  % bar moving out occurs during ITI but is queued with previous trial
            startTrig = numITITriggers-maxNumStepTriggers-numLinTriggers+1;                    % move during end of ITI but leave room for bar moving out
            endTrig = Experiment.timing.numScansPerTrial-maxNumStepTriggers-numLinTriggers+1;  % bar moving out occurs during ITI but is queued with previous trial
            ...
            Experiment.Triggers([startTrig, endTrig], strcmp(OutChannels,'O_EventTrigger'), p) = 1; % trigger at beginning and end of stimulus
...
%             beginMove = stopMove-numStepTriggers+1;
            stopMove = startTrig-1;
            beginMove = stopMove-numStepTriggers+1;
            Experiment.Triggers(beginMove:stopMove, strcmp(OutChannels,'O_MotorStep'), p) = stepTriggers(:,1);
%             if Experiment.params.catchTrials
...
%             stopMove = Experiment.timing.numScansPerTrial;
            beginMove = Experiment.timing.numScansPerTrial-maxNumStepTriggers+1;
            stopMove = Experiment.timing.numScansPerTrial-maxNumStepTriggers+numStepTriggers;
            Experiment.Triggers(beginMove:stopMove, strcmp(OutChannels,'O_MotorStep'), p) = stepTriggers(:,1);
            Experiment.Triggers(beginMove:stopMove-1, strcmp(OutChannels,'O_MotorDir'), p) = 5;
%             if Experiment.params.catchTrials
%                 Experiment.Triggers(beginMove:stopMove, strcmp(OutChannels,'O_MotorStep'), 2) = stepTriggers(:,1);
%             end
            ...
            if strcmp(Experiment.imaging.ImagingMode,'Trial Imaging')
                ...
                Experiment.Triggers([delay, stopMove], strcmp(OutChannels, 'O_ImagingTrigger'), p) = 1;
            end
            
            % Trigger whisker tracking camera on every single trial
            if Experiment.params.whiskerTracking
                if ~gd.Parameters.wtType.Value	% single trigger per frame
                    Experiment.Triggers(startTrig:ceil(DAQ.Rate/Experiment.params.frameRateWT):endTrig, strcmp(OutChannels,'O_WhiskerTracker'), p) = 1; % image during stimulus period
                else                            % single trigger per trial
                    Experiment.Triggers(startTrig, strcmp(OutChannels,'O_WhiskerTracker'), p) = 1; % image during stimulus period
                end
            end
        end
...
%         ActiveAxes = find(Experiment.stim.activeAxes);
        ...
        Distances = Experiment.stim.Distances;
        preppedTrial = false;
...
%% Callback: DataIn
    function SaveDataIn(src,eventdata)
        ...
        ylim([-100,600]);
        if any(diff(BufferStim(numBufferScans-numScansReturned:numBufferScans)) == RunIndex) % stimulus started during current DataIn call
            if TrialInfo.StimID(RunIndex)==0        % this trial is a control trial
%                 temp = randi([2,numStimuli]);
                moveLinearMotor(Distances(3), H_LinearStage(2)); %move ML motor
            else                                    % next trial is not a control trial
                moveLinearMotor(Distances(2), H_LinearStage(2)); %move ML motor
            end
        end
        ...
        if any(diff(BufferStim(numBufferScans-numScansReturned:numBufferScans)) == -RunIndex) % stimulus ended during current DataIn call
            moveLinearMotor(Distances(1), H_LinearStage(2)); %move ML motor
            ...
            % return to baseline cassette rotation
%             DAQ.queueOutputData(Triggers(:,:,index));
...
%% Callback: QueueOutputData
    function QueueData(src,eventdata)
        ...
        if hObject.Value && any(numTrialsRemain) % user hasn't quit, and experiment hasn't finished
            ...
            index = TrialInfo.StimID(currentTrial)+1;
...
%                     end
%                 end
                moveLinearMotor(Distances(1),H_LinearStage(2));
