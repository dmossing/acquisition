...
function run_visual_stim_sweeps(varargin)
...
if strcmp(modality,'2p')
    resDir = [stimfolder dstr '\' result.animalid '\2P\'];
else
    resDir = [stimfolder dstr '\' result.animalid '\LF\'];
end
if ~exist(resDir,'dir')
    mkdir(resDir)
end
...
if strcmp(modality,'2p')
    ...
    lfpath = 'E:/LF2P/Dan/';
    lffolder = [lfpath dstr '/' base];
    %     wiggle_room = 1;
    %     pretrig = isi-stimduration-wiggle_room;
    %     posttrig = stimduration+wiggle_room;
end
...
Screen('DrawText', w, strcat(num2str(allConds),' Conditions__',num2str(result.repetitions),...
    ' Repeats__',num2str(totalsec/60),...
...
if kinp == 'q'|kinp == 'Q',
    ...
    if strcmp(modality,'2p')
        ...
        fprintf(H_lf,sprintf('G%s/;%s_%s_%s;%d;', lffolder, base, depth, fileindex,totalsec));
    end
    pause(5);
...
%                 end
                ...
                %                 else
                %                     WaitSecs(max(0,pretrig-((GetSecs-t0)-trialstart)));
                %                     outputSingleScan(daq,[0 1 0])
                %                     outputSingleScan(daq,[1 1 0])
                %                     outputSingleScan(daq,[0 1 0])
                %                     WaitSecs(max(0,posttrig));
                %                 end
...
%     else
%         outputSingleScan(daq,[0 0 0]) % extra trig to say stim has ended for lf
