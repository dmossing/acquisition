function run_movie_expt_triggered(varargin)
...
result = p.Results;
result.numberofstims = result.numToTrigOn;
...
nConds = [length(result.numberofstims) length(result.contrast)];
...
makeAllCombos(result.numberofstims,result.contrast);
...
[gratingInfo.Contrast,gratingInfo.whichClip] = deal(zeros(1,2*allConds*result.repetitions));
...
% gratingInfo.gtype = 'box';
        ...
        for i=1:result.numberofstims
%             gi.Orientation = result.orientations(i);
%             thisstim = getStim(gi,1);
            gi.whichClip = i;
            thisstim = getStim(gi,1);
            thisstim.tex = gen_textures(wininfo,stims{thisstim.whichClip});
...
%         result.gratingInfo.Contrast(trnum) = thiscond(5);
        result.gratingInfo.whichClip(trnum) = thiscond(1);
        result.gratingInfo.Contrast(trnum) = thiscond(2);
    end
...
%         thisstim.thisfreq = gratingInfo.spFreq(trnum);
        thisstim.thisclip = gratingInfo.whichClip(trnum);
